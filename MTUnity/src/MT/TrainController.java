...
public class TrainController 
{
	...
	public void addPair(ArrayList<String[]> eng,ArrayList<String[]> esp)
	{
		...
		int mayor = Math.max(eng.size(), esp.size());
		int dif = mayor-min;
		...
		if(dif>=0)
		{
			ArrayList<String[]> max = (eng.size()>= esp.size()?eng:esp);
	...
	{
		HashMap<String[], HashMap<String[], Integer>> actual = pair.getNEnglish();
		//System.out.println("PROBS SIZE : "+actual.size());
	}
	
	public PairNGram getTrainPairs()
	{
		System.out.println("It gets train pairs: "+pair);
		return pair;
	}
